...
class Spring {
  ...
  computeForce(Body b1, Body b2) return Vector {
...
class Utils {
  static dot(float<2> v1, float<2> v2) return float {
    var r = v1 * v2;
    return r.x + r.y;
  }
  static length(float<2> v) return float {
    return Math.sqrt(Utils.dot(v, v));
  }
  static dot(float<3> v1, float<3> v2) return float {
    var r = v1 * v2;
    return r.x + r.y + r.z;
  }
  static length(float<3> v) return float {
    return Math.sqrt(Utils.dot(v, v));
  }
  static makeFloat4(float<2> v) return float<4> {
    return float<4>(v.x, v.y, 0.0, 1.0);
  }
  static makeFloat4(float<3> v) return float<4> {
    return float<4>(v.x, v.y, v.z, 1.0);
  }
  static makeVector(float x, float y, float z, float<2> placeholder) return float<2> {
    return float<2>(x, y);
  }
  static makeVector(float x, float y, float z, float<3> placeholder) return float<3> {
