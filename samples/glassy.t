...
auto texture = new sampleable TextureCube<RGBA8unorm>(device, {1792, 1792});
CubeLoader.Load(device, inline("third_party/home-cube/diocletians-palace/right.jpg"), texture, 0);
CubeLoader.Load(device, inline("third_party/home-cube/diocletians-palace/left.jpg"), texture, 1);
CubeLoader.Load(device, inline("third_party/home-cube/diocletians-palace/top.jpg"), texture, 2);
CubeLoader.Load(device, inline("third_party/home-cube/diocletians-palace/bottom.jpg"), texture, 3);
CubeLoader.Load(device, inline("third_party/home-cube/diocletians-palace/front.jpg"), texture, 4);
CubeLoader.Load(device, inline("third_party/home-cube/diocletians-palace/back.jpg"), texture, 5);
...
auto tessTeapot = new BicubicTessellator(&spheroidControlPoints, &spheroidControlIndices, 64);
...
class ReflectionPipeline : Pipeline {
    ...
    void fragmentShader(FragmentBuiltins fb, Vertex varyings) fragment {
      ...
      float<3> r = Math.refract(p, n, 0.9);
...
auto spheroidPipeline = new RenderPipeline<ReflectionPipeline>(device, depthState, TriangleList);
Bindings spheroidBindings;
spheroidBindings.sampler = cubeBindings.sampler;
spheroidBindings.textureView = cubeBindings.textureView;
spheroidBindings.uniforms = new uniform Buffer<Uniforms>(device);

ReflectionPipeline spheroidData;
spheroidData.vert = new vertex Buffer<Vertex[]>(device, tessTeapot.vertices);
spheroidData.indexBuffer = new index Buffer<uint[]>(device, tessTeapot.indices);
spheroidData.bindings = new BindGroup<Bindings>(device, &spheroidBindings);

EventHandler handler;
handler.rotation = float<2>(0.0, 0.0);
handler.distance = 10.0;
auto spheroidQuat = Quaternion(float<3>(1.0, 0.0, 0.0), -3.1415926 / 2.0);
spheroidQuat.normalize();
auto spheroidRotation = spheroidQuat.toMatrix();
...
//  uniforms.viewInverse = Transform.invert(uniforms.view);
  ...
  uniforms.model = spheroidRotation * Transform.scale(2.0, 2.0, 2.0);
  spheroidBindings.uniforms.SetData(&uniforms);
  auto encoder = new CommandEncoder(device);
  ...
  auto spheroidPass = new RenderPass<ReflectionPipeline>(renderPass);
  spheroidPass.SetPipeline(spheroidPipeline);
  spheroidPass.Set(&spheroidData);
  spheroidPass.DrawIndexed(tessTeapot.indices.length, 1, 0, 0, 0);
