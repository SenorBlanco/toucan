...
PrintAST::PrintAST(TypeTable* types) : types_(types) {}
...
Result PrintAST::Visit(HeapAllocation* node) {
  Output(node, "HeapAllocation %s", node->GetType()->ToString().c_str());
  if (node->GetLength()) Resolve(node->GetLength());
  return {};
}
...
Result PrintAST::Visit(TempVarExpr* node) {
  ...
  if (node->GetInitExpr()) Resolve(node->GetInitExpr());
  return {};
}
...
static const char* GetBinOp(BinOpNode::Op op) {
  switch (op) {
...
static const char* GetUnaryOp(UnaryOp::Op op) {
  switch (op) {
    case UnaryOp::Op::Minus: return "Minus";
    case UnaryOp::Op::Negate: return "Negate";
    default: assert(false); return "";
  }
}

Result PrintAST::Visit(BinOpNode* node) {
  Output(node, "BinOpNode %s", GetBinOp(node->GetOp()));
  Resolve(node->GetLHS());
  Resolve(node->GetRHS());
  return {};
}
...
Result PrintAST::Visit(RawToSmartPtr* node) {
  Output(node, "RawToSmartPtr");
  Resolve(node->GetExpr());
  return {};
}
...
Result PrintAST::Visit(ToRawArray* node) {
  Output(node, "ToRawArray %s memoryLayout %d", node->GetType(types_)->ToString().c_str(), node->GetMemoryLayout());
  Resolve(node->GetLength());
  Resolve(node->GetData());
  return {};
}

Result PrintAST::Visit(UnaryOp* node) {
  Output(node, "UnaryOp %s", GetUnaryOp(node->GetOp()));
  Resolve(node->GetRHS());
  return {};
}
...
Result PrintAST::Visit(WhileStatement* node) {
  Expr*             cond = node->GetCond();
  Output(node, "WhileStatement");
  Resolve(node->GetCond());
  if (node->GetBody()) Resolve(node->GetBody());
  return {};
}
